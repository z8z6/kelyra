...
首个可交付版本只实现静态分派、单一构造函数和可选析构函数。不包含继承、虚函数、
异常、运算符重载、隐式堆分配或隐式复制。这些能力不是 RAII 成立的前提。
...
一个 class 必须恰有一个 `init`，最多有一个 `deinit`。没有 `deinit` 时编译器生成
空析构体，但仍会析构 class 类型的字段。首版不提供构造函数重载；替代构造路径写成
接受对象指针的普通模块函数。
...
构造表达式不是可任意复制的普通右值，而是一个“初始化目标存储”的操作。首版只允许
出现在以下位置：
...
编译器先为对象分配最终存储，再以该地址作为 `this` 调用 `init`，不会建立隐藏临时对象。
`let file: File;` 非法，class 局部变量必须通过其构造函数初始化。
...
赋值表达式初始化；class 字段使用构造表达式直接在字段地址构造。初始化完成后，普通
字段可以继续赋值。对已构造 class 对象或 class 字段的整体赋值禁止，以免出现隐式复制
和漏析构；应修改其字段或通过指针操作对象。

按值递归包含自身的 class 非法，`*Self` 字段合法。首版支持 class 字段，但不支持
class 数组、class 按值参数或任意 class 返回值。
...
首版 class 是不可隐式复制的资源值：

- class 整体赋值非法；
- class 按值传参和普通按值返回非法；
- 构造表达式只能直接初始化最终存储；
- 指针 `*Class` 可以传递和保存，但不转移析构责任。

这条限制避免位复制导致双重释放，也避免在尚无成熟所有权模型时引入隐藏 move。需要
共享或转移资源的库先显式提供句柄、指针或 `take`/`release` 风格 API。只有真实用例需要
class 容器和按值返回时，再单独设计复制/移动构造函数及对应诊断。
...
- 缺失或重复的 `init`、重复的 `deinit`；
- 字段漏初始化、重复初始化、顺序错误或初始化前读取；
- 直接调用 `deinit`，或构造表达式出现在没有目标存储的位置；
- class 整体赋值、按值参数、普通按值返回和 class 数组；
...
前五项已实现；后续根据真实标准库需求决定是否加入 class 数组、按值返回以及显式复制/移动。
